...
void Renderer::GenerateVertices(v_3ct *vertices, UINT &index, Drawable* obj){
  if (auto complex_obj = dynamic_cast<Complex *>(obj)) {
    ...
    vertices[index * 4] = {0.0f, 0.0f, 0.0f, textured_quad->color_[3], 0.0f, 1.0f};
    vertices[index * 4 + 1] = {0.0f, textured_quad->size().height, 0.0f, textured_quad->color_[0], 0.0f, 0.0f};
    vertices[index * 4 + 2] = {textured_quad->size().width, 0.0f, 0.0f, textured_quad->color_[2], 1.0f, 1.0f};
    vertices[index * 4 + 3] =
        {textured_quad->size().width, textured_quad->size().height, 0.0f, textured_quad->color_[1], 1.0f, 0.0f};
    index++;
  }
}
